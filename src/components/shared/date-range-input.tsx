...
function toPickerRange(value: DateRangeValue): DayPickerRange {
  return {
...
function toStringRange(range: DayPickerRange | undefined): DateRangeValue {
  return {
...
function formatLabel(value: DateRangeValue, placeholder: string): string {
  ...
  if (value.from && value.to) {
    return `${format(new Date(value.from), "MMM d")} – ${format(
      new Date(value.to),
      "MMM d, yyyy",
    )}`;
  }
